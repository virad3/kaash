...
import { Transaction, ExpenseCategory, TransactionType, Liability, SavingsGoal } from '../types';
...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index, name, x, y, ...props }: any) => {
  ...
  const thresholdPercent = 0.05; // Only show label if segment is > 5%

  if (percent < thresholdPercent) {
    return null; // Hide label for small segments
  }
  
  // Check screen width for label content - a bit hacky without a proper hook here, but for demonstration
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 640;


  return (
    <text 
      x={isMobile ? (cx + (outerRadius + 10) * Math.cos(-midAngle * RADIAN)) : labelX} // Position label outside for mobile
...
export const SummaryDisplay: React.FC<SummaryDisplayProps> = ({ totalIncome, totalExpenses, balance, expenseTransactions, liabilities, savingsGoals }) => {
  ...
  const totalCurrentSavings = useMemo(() => {
    return savingsGoals.reduce((sum, sg) => sum + sg.currentAmount, 0);
  }, [savingsGoals]);
    ...
    <div className="bg-slate-800 p-3 sm:p-6 rounded-xl shadow-xl border border-slate-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-slate-700/50 p-3 sm:p-4 lg:p-6 rounded-lg shadow-md text-center">
         ...
         <div className="bg-slate-700/50 p-3 sm:p-4 lg:p-6 rounded-lg shadow-md text-center">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-teal-400">₹{totalCurrentSavings.toFixed(2)}</p>
        </div>
        <div className="bg-slate-700/50 p-3 sm:p-4 lg:p-6 rounded-lg shadow-md text-center sm:col-span-2 lg:col-span-1 xl:col-span-2"> {/* Span for larger screens */}
      ...
      {expenseDataByCategory.length > 0 && (
         <div className="mt-6 sm:mt-8">
            ...
            <div style={{ width: '100%', height: 300, minHeight: '250px' }}> {/* Added minHeight */}
                <ResponsiveContainer>
                    <PieChart margin={{ top: 0, right: 0, bottom: 20, left: 0 }}>
                        <Pie
                            ...
                            cy="50%"
                            labelLine={false}
                            outerRadius="80%" // Adjusted outerRadius for better fit
                            fill="#8884d8"
                            dataKey="value"
                            nameKey="name"
                        ...
                        <Legend 
                           ...
                           payload={
                            expenseDataByCategory.map(
                              (entry, index) => ({
                                value: entry.name.length > 12 ? `${entry.name.substring(0,10)}...` : entry.name, // Truncate legend text
                                type: 'circle',
                                id: entry.name,
