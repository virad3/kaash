...
export const SavingsDetailsPage: React.FC<SavingsDetailsPageProps> = ({ savingTransactions, onBack, onEditTransaction, onDeleteTransaction }) => {
  ...
  const filteredTransactions = useMemo(() => {
    return savingTransactions.filter(t => {
      const monthMatch = selectedMonth === 'all' || t.date.startsWith(selectedMonth);
      const categoryMatch = selectedCategory === 'all' || t.category === selectedCategory;
      return monthMatch && categoryMatch;
    });
  }, [savingTransactions, selectedMonth, selectedCategory]);

  const totalFilteredSavings = useMemo(() => {
    return filteredTransactions.reduce((sum, t) => sum + t.amount, 0);
  }, [filteredTransactions]);

  const savingChartData = useMemo(() => {
    const categoryMap: { [key: string]: number } = {};
    filteredTransactions.forEach(transaction => {
      if (transaction.type === TransactionType.SAVING && transaction.category) {
