...
interface SignupFormProps {
  onSignup: (name: string, email: string, password?: string) => void; // Password optional for demo
  onSwitchToLogin: () => void;
  error?: string | null;
}

export const SignupForm: React.FC<SignupFormProps> = ({ onSignup, onSwitchToLogin, error }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Add password validation if it's required for your demo logic
    onSignup(name, email, password);
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <h2 className="text-3xl font-bold text-center text-sky-400">Create Kaash Account</h2>
      {error && <p className="text-red-400 text-sm text-center bg-red-500/10 p-2 rounded-md">{error}</p>}
      <div>
        <label htmlFor="signup-name" className="block text-sm font-medium text-gray-300 mb-1">Full Name</label>
        <input
          type="text"
          id="signup-name"
          value={name}
      ...
      <div>
        <label htmlFor="signup-email" className="block text-sm font-medium text-gray-300 mb-1">Email Address</label>
        <input
          type="email"
          id="signup-email"
          value={email}
      ...
      <div>
        ...
        <input
          ...
          // required // Making password optional for demo to simplify
        />
      ...
      <p className="text-sm text-center text-gray-400">
        ...
        <button type="button" onClick={onSwitchToLogin} className="font-medium text-sky-400 hover:text-sky-300">
          Log in
        </button>
      </p>
    </form>
